...
const AdminPanel: React.FC = () => {
  ...
  const handleSyncPlayerStatus = async () => {
    CustomAlertManager.alert(
      '🏥 Sincronizar Estado de Jugadores',
      '¿Actualizar el estado de disponibilidad de todos los jugadores?\n\n' +
      'Esto consultará la API de Football para detectar:\n' +
          ...
          onPress: async () => {
            try {
              ...
              const response = await axios.post(
                `${ApiConfig.BASE_URL}/players/status/sync`,
                {},
                {
                  headers: {
              ...
              CustomAlertManager.alert(
                ...
                `Estado de jugadores actualizado correctamente.\n\n` +
                `Los cambios ya están disponibles en el mercado.`,
                [{ 
                  text: 'OK', 
                  onPress: () => {}, 
                  style: 'default' 
      ...
      >
        ...
        >
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
            <View style={{ marginRight: 12 }}>
              <InjuryIcon size={32} color="#0892D0" />
            </View>
            <Text
              style={{
                color: '#fff',
          ...
          >
            Actualiza el estado de disponibilidad de todos los jugadores (lesiones y suspensiones) desde la API de Football.
          </Text>

          <TouchableOpacity
            onPress={handleSyncPlayerStatus}
            disabled={isSyncingPlayers}
            style={{
              backgroundColor: isSyncingPlayers ? '#334155' : '#0892D0',
              paddingVertical: 16,
              borderRadius: 12,
              alignItems: 'center',
              shadowColor: '#0892D0',
              shadowOffset: { width: 0, height: 4 },
              shadowOpacity: isSyncingPlayers ? 0 : 0.3,
              shadowRadius: 8,
              elevation: isSyncingPlayers ? 0 : 4,
            }}
          >
            {isSyncingPlayers ? (
