 ...
 * 
 * NOTA: Este archivo se sincroniza con ../shared/pointsCalculator.ts del proyecto raíz
 */
...
export { normalizeRole, CLEAN_SHEET_MINUTES };
export type { Role };
...
export function calculatePlayerPoints(stats: any, role: Role): PointsResult {
  ...
  if (role === 'Goalkeeper') {
    ...
    if (goalsScored) add('Goles marcados', goalsScored, goalsScored * GOALKEEPER_POINTS.GOAL);
    ...
    // Goles marcados
    const goalsScored = goals.total || 0;
    if (goalsScored) add('Goles marcados', goalsScored, goalsScored * DEFENDER_POINTS.GOAL);
    ...
    // Goles marcados
    const goalsScored = goals.total || 0;
    if (goalsScored) add('Goles marcados', goalsScored, goalsScored * MIDFIELDER_POINTS.GOAL);
    ...
    if (conceded > 0) {
      const concededPoints = -Math.floor(conceded / MIDFIELDER_POINTS.GOALS_CONCEDED_PER_PENALTY);
    ...
    // Goles marcados
    const goalsScored = goals.total || 0;
    if (goalsScored) add('Goles marcados', goalsScored, goalsScored * ATTACKER_POINTS.GOAL);
  ...
  if (rawRating != null && rawRating !== '') {
    ...
    if (!Number.isNaN(rating)) {
      let ratingPoints = 0;
      if (rating >= 9) {
        ratingPoints = BASE_POINTS.RATING_9_PLUS;
      } else if (rating >= 8) {
