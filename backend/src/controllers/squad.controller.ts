...
export class SquadController {
  ...
  static async getUserBudget(req: FastifyRequest<{ Params: { ligaId: string } }>, reply: FastifyReply) {
    try {
      const { ligaId } = req.params;
      const userId = (req.user as any)?.sub || (req.user as any)?.id;
  ...
  static async addPlayerToSquad(req: FastifyRequest<{ 
    Params: { ligaId: string }; 
    Body: { position: string; playerId: number; playerName: string; role: string; pricePaid: number } 
  }>, reply: FastifyReply) {
    try {
      const { ligaId } = req.params;
      const userId = (req.user as any)?.sub || (req.user as any)?.id;
  ...
  static async removePlayerFromSquad(req: FastifyRequest<{ 
    Params: { ligaId: string; position: string } 
  }>, reply: FastifyReply) {
    try {
