...
async function checkDatabaseState() {
    try {
        ...
        for (let j = 1; j <= 15; j++) {
            ...
            if (bets.length > 0) {
                ...
                for (const bet of bets) {
                    console.log(`    - Usuario ${bet.leagueMember.user.name}: ${bet.betType} - ${bet.betLabel} ` +
        ...
        for (const squad of squads) {
            console.log(`  - Usuario ${squad.user.name} en liga ${squad.league.name}: ` +
        ...
        for (const member of members) {
            console.log(`  - ${member.user.name} en ${member.league.name}:\n` +
