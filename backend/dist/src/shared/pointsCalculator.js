 ...
 */
import { BASE_POINTS, GOALKEEPER_POINTS, DEFENDER_POINTS, MIDFIELDER_POINTS, ATTACKER_POINTS, CLEAN_SHEET_MINUTES, normalizeRole, } from './pointsConfig.js';
...
export function calculatePlayerPoints(stats, role) {
    ...
    if (minutesPoints !== 0)
        add('Minutos jugados', minutes, minutesPoints);
    ...
    if (goals.assists)
        add('Asistencias', goals.assists, goals.assists * BASE_POINTS.ASSIST);
    ...
    if (penalty.won)
        add('Penaltis ganados', penalty.won, penalty.won * BASE_POINTS.PENALTY_WON);
    if (penalty.committed)
        add('Penaltis cometidos', penalty.committed, penalty.committed * BASE_POINTS.PENALTY_COMMITTED);
    // ✅ ELIMINADO: Penaltis marcados ya cuentan como goles normales
    if (penalty.missed)
        add('Penaltis fallados', penalty.missed, penalty.missed * BASE_POINTS.PENALTY_MISSED);
    // ========== PUNTOS ESPECÍFICOS POR POSICIÓN ==========
    if (role === 'Goalkeeper') {
        // Goles marcados
        const goalsScored = goals.total || 0;
        if (goalsScored)
            add('Goles marcados', goalsScored, goalsScored * GOALKEEPER_POINTS.GOAL_SCORED);
        ...
        if (saves)
            add('Paradas', saves, saves * GOALKEEPER_POINTS.SAVE);
        // Penaltis parados
        ...
        const goalsScored = goals.total || 0;
        if (goalsScored)
            add('Goles marcados', goalsScored, goalsScored * DEFENDER_POINTS.GOAL_SCORED);
        ...
        if (dribblePoints)
            add('Regates exitosos', dribblesSuccess, dribblePoints);
        // Faltas recibidas
        ...
        const goalsScored = goals.total || 0;
        if (goalsScored)
            add('Goles marcados', goalsScored, goalsScored * ATTACKER_POINTS.GOAL_SCORED);
        // Tiros a puerta
        const shotsOn = Number(shots.on || 0);
        if (shotsOn)
            add('Tiros a puerta', shotsOn, shotsOn * ATTACKER_POINTS.SHOT_ON_TARGET);
        // Pases clave
        const passesKey = Number(passes.key || 0);
 ...
 */
export function calculatePlayerPointsTotal(stats, role) {
