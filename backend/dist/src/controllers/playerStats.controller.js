...
export class PlayerStatsController {
     ...
     */
    static async getPlayerJornadaStats(req, reply) {
        try {
            const { playerId, jornada } = req.params;
            const query = req.query;
            const stats = await PlayerStatsService.getPlayerStatsForJornada(Number(playerId), Number(jornada), {
     ...
     */
    static async getPlayerMultipleJornadasStats(req, reply) {
        try {
            ...
            const jornadas = body.jornadas
                .map((j) => Number(j))
                .filter((j) => Number.isInteger(j) && j > 0 && j <= 38);
            if (!jornadas.length) {
                return reply.status(400).send({
                    success: false,
            ...
            const stats = await PlayerStatsService.getPlayerStatsForMultipleJornadas(Number(playerId), jornadas, {
     ...
     */
    static async updateJornadaStats(req, reply) {
        try {
            const body = req.body;
