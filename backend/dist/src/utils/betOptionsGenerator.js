...
function translateBetType(betType) {
    ...
    if (lowerType.includes('win to nil') || lowerType.includes('win to 0')) {
        if (lowerType.includes('home'))
            return 'Local Gana sin Encajar';
        if (lowerType.includes('away'))
            return 'Visitante Gana sin Encajar';
        return 'Ganar sin Encajar';
    }
    // Highest Scoring
    if (lowerType.includes('highest scoring')) {
        if (lowerType.includes('half'))
            return 'Parte con Más Goles';
        if (lowerType.includes('2nd') || lowerType.includes('second'))
            return 'Segunda Parte con Más Goles';
        if (lowerType.includes('1st') || lowerType.includes('first'))
            return 'Primera Parte con Más Goles';
    }
    // Goles
    if (lowerType.includes('goals')) {
        if (lowerType.includes('over') || lowerType.includes('under')) {
            if (lowerType.includes('first half') || lowerType.includes('1st half'))
                return 'Más/Menos Goles Primera Parte';
            if (lowerType.includes('second half') || lowerType.includes('2nd half'))
                return 'Más/Menos Goles Segunda Parte';
            return 'Más/Menos Goles';
        }
        if (lowerType.includes('total')) {
            if (lowerType.includes('home'))
                return 'Total Goles Local';
            if (lowerType.includes('away'))
                return 'Total Goles Visitante';
            if (lowerType.includes('first half') || lowerType.includes('1st half'))
                return 'Total Goles Primera Parte';
            if (lowerType.includes('second half') || lowerType.includes('2nd half'))
                return 'Total Goles Segunda Parte';
            return 'Total de Goles';
        }
        if (lowerType.includes('multigoals')) {
            if (lowerType.includes('home'))
                return 'Multigoles Local';
            if (lowerType.includes('away'))
                return 'Multigoles Visitante';
            return 'Multigoles';
        }
    }
    // Corners
    if (lowerType.includes('corners') || lowerType.includes('corner')) {
        if (lowerType.includes('over') || lowerType.includes('under')) {
            if (lowerType.includes('first half') || lowerType.includes('1st half'))
                return 'Más/Menos Corners Primera Parte';
            if (lowerType.includes('second half') || lowerType.includes('2nd half'))
                return 'Más/Menos Corners Segunda Parte';
            return 'Más/Menos Corners';
        }
        if (lowerType.includes('total')) {
            if (lowerType.includes('home'))
                return 'Total Corners Local';
            if (lowerType.includes('away'))
                return 'Total Corners Visitante';
            return 'Total de Corners';
        }
        if (lowerType.includes('home'))
            return 'Corners del Local';
        if (lowerType.includes('away'))
            return 'Corners del Visitante';
    }
    // Tarjetas
    if (lowerType.includes('cards') || lowerType.includes('card')) {
        ...
        if (lowerType.includes('total')) {
            if (lowerType.includes('home'))
                return 'Total Tarjetas Local';
            if (lowerType.includes('away'))
                return 'Total Tarjetas Visitante';
            return 'Total de Tarjetas';
        }
        if (lowerType.includes('home'))
            return 'Tarjetas del Local';
        if (lowerType.includes('away'))
            return 'Tarjetas del Visitante';
        if (lowerType.includes('player'))
            return 'Tarjetas de Jugadores';
    }
    // Ambos marcan
    ...
    if (lowerType.includes('clean sheet')) {
        if (lowerType.includes('home'))
            return 'Portería a Cero - Local';
        if (lowerType.includes('away'))
            return 'Portería a Cero - Visitante';
        return 'Portería a Cero';
    }
    // Ganador
    if (lowerType.includes('winner')) {
        if (lowerType.includes('first half') || lowerType.includes('1st half'))
            return 'Ganador Primera Parte';
        if (lowerType.includes('second half') || lowerType.includes('2nd half'))
            return 'Ganador Segunda Parte';
        return 'Ganador del Partido';
    }
    // Resultado
    if (lowerType.includes('result')) {
        if (lowerType.includes('halftime') || lowerType.includes('half time'))
            return 'Resultado al Descanso';
        if (lowerType.includes('fulltime') || lowerType.includes('full time'))
            return 'Resultado Final';
        if (lowerType.includes('handicap'))
            return 'Resultado con Hándicap';
    }
    // Par/Impar
    if ((lowerType.includes('odd') && lowerType.includes('even')) || lowerType.includes('odd/even')) {
        if (lowerType.includes('home'))
            return 'Goles Par/Impar Local';
        if (lowerType.includes('away'))
            return 'Goles Par/Impar Visitante';
    ...
    if (lowerType.includes('win')) {
        if (lowerType.includes('both halves') || lowerType.includes('both half'))
            return 'Ganar Ambas Partes';
        if (lowerType.includes('either half') || lowerType.includes('either half'))
            return 'Ganar Alguna Parte';
        if (lowerType.includes('from behind'))
            return 'Ganar Remontando';
    }
    // Marcador
    if (lowerType.includes('score')) {
        if (lowerType.includes('first') || lowerType.includes('1st'))
            return 'Marcar Primero';
        if (lowerType.includes('last'))
            return 'Marcar Último';
        if (lowerType.includes('both halves'))
            return 'Marcar en Ambas Partes';
    }
    return betType;
}
function translateBetLabel(betLabel, homeTeam, awayTeam) {
    ...
    if (betLabel.toLowerCase().includes('clean sheet')) {
        if (betLabel.toLowerCase().includes('home'))
            return 'Sí - Local';
        if (betLabel.toLowerCase().includes('away'))
            return 'Sí - Visitante';
        if (betLabel.toLowerCase().includes('yes'))
            return 'Sí';
        if (betLabel.toLowerCase().includes('no'))
            return 'No';
    }
    // Si es un nombre de equipo, mantenerlo tal cual
    if (homeTeam && betLabel === homeTeam)
        return homeTeam;
    if (awayTeam && betLabel === awayTeam)
        return awayTeam;
    // Si contiene el nombre de un equipo, mantenerlo
    if (homeTeam && betLabel.includes(homeTeam))
        return betLabel;
    if (awayTeam && betLabel.includes(awayTeam))
        return betLabel;
    return betLabel;
}
    ...
    try {
        ...
        bookmaker.bets.forEach((bet) => {
            ...
            const isMatchWinner = betNameLower.includes('match winner') ||
                ...
                (bet.values.length === 3 &&
                    bet.values.some((v) => v.value === 'Home') &&
                    bet.values.some((v) => v.value === 'Draw') &&
                    bet.values.some((v) => v.value === 'Away'));
