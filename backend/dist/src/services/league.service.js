...
export const LeagueService = {
     ...
     */
    getAllClassifications: async (leagueId) => {
        const league = await LeagueRepo.getById(leagueId);
        if (!league) {
            throw new Error('Liga no encontrada');
        }
            ...
            if (Object.keys(pointsPerJornada).length === 0) {
                ...
                Object.keys(pointsPerJornada).forEach(jornadaStr => {
                    ...
                    if (jornada >= 1 && jornada <= 38) {
                        const memberIndex = classifications[jornada].findIndex((m) => m.userId === member.userId);
                        if (memberIndex >= 0) {
        ...
        if (jornadaStatus === 'closed' && currentJornada >= 1 && currentJornada <= 38) {
            ...
            const realTimePoints = await Promise.all(members.map(async (member) => {
                try {
                    ...
                    const captainPlayer = squad.players.find((p) => p.isCaptain);
                    if (captainPlayer) {
            ...
            realTimePoints.forEach(({ userId, points }) => {
                const memberIndex = classifications[currentJornada].findIndex((m) => m.userId === userId);
                if (memberIndex >= 0) {
        ...
        for (let j = 1; j <= 38; j++) {
            classifications[j].sort((a, b) => b.points - a.points);
        }
        return {
            leagueId: league.id,
     ...
     */
    listMembersByJornada: async (leagueId, jornada) => {
        const prisma = new PrismaClient();
        try {
            // Obtener información de la liga
            const league = await LeagueRepo.getById(leagueId);
            ...
            const membersWithJornadaPoints = await Promise.all(members.map(async (member) => {
                try {
                    // Obtener la plantilla del usuario en esta liga
                    const squad = await prisma.squad.findUnique({
                        where: {
     ...
     */
    calculateRealTimePoints: async (leagueId) => {
        try {
            ...
            await Promise.all(members.map(async (member) => {
                ...
                if (squad && squad.players.length === 11) {
                    ...
                    squad.players.forEach((p) => allPlayerIds.add(p.playerId));
                }
                else if (squad) {
            ...
            await Promise.all(Array.from(allPlayerIds).map(async (playerId) => {
                try {
            ...
            const userPoints = await Promise.all(members.map(async (member) => {
                ...
                let sumPoints = 0;
                const captainId = squad.players.find((p) => p.isCaptain)?.playerId || null;
                const playersWithPoints = playerStats.map((stats) => {
                    const points = stats.totalPoints || 0;
