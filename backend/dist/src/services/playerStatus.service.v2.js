 ...
 */
export async function getPlayerAvailabilityFromAPI(playerId, season = 2025) {
    try {
 ...
 */
async function checkRedCardSuspension(playerId, season) {
    try {
        ...
        const totalYellows = recentStats.reduce((sum, stats) => sum + (stats.yellowCards || 0), 0);
        if (totalYellows >= 5) {
            return {
                status: 'SUSPENDED',
 ...
 */
export async function updatePlayerAvailability(playerId, season = 2025) {
    try {
 ...
 */
export async function updateAllPlayersAvailability(season = 2025) {
    try {
        ...
        for (const player of players) {
            ...
            if (updated?.availabilityStatus === 'AVAILABLE')
                available++;
            else if (updated?.availabilityStatus === 'INJURED')
                injured++;
            else if (updated?.availabilityStatus === 'SUSPENDED')
                suspended++;
            await delay(400);
            if (processed % 10 === 0) {
