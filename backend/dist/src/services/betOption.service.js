...
export class BetOptionService {
    ...
    static async generateBetOptions(leagueId, jornada) {
        ...
        try {
            ...
            let oddsMap = new Map(); // matchId -> odds data
            try {
            ...
            for (const fixture of fixtures) {
                ...
                if (bets.length > 0) {
                    ...
                    for (const bet of bets) {
                        ...
                        let matches = false;
                        if (selectedBetType === 'Resultado' && betId === 1)
                            matches = true;
                        else if (selectedBetType === 'Goles totales' && betId === 5)
                            matches = true;
                        else if (selectedBetType === 'Ambos marcan' && betId === 8)
                            matches = true;
                        else if (selectedBetType === 'Córners' && betId === 61)
                            matches = true;
                        else if (selectedBetType === 'Tarjetas' && betId === 52)
                            matches = true;
                        if (matches) {
                            ...
                            for (const v of values) {
                                ...
                                if (selectedBetType === 'Resultado') {
                                    if (v.value === 'Home')
                                        label = `Ganará ${homeTeam}`;
                                    else if (v.value === 'Draw')
                                        label = 'Empate';
                                    else if (v.value === 'Away')
                                        label = `Ganará ${awayTeam}`;
                                }
