  ...
  try {
    ...
    if (stats) {
      ...
      if (response.data.response && response.data.response.length > 0) {
        ...
        const playerEvents = eventsResponse.data.response.filter((e: any) => 
        ...
        if (playerEvents.length > 0) {
          ...
          playerEvents.forEach((e: any) => {
            ...
            if (e.player) console.log(`     Jugador: ${e.player.name}`);
            if (e.assist) console.log(`     Asistente/Entrada: ${e.assist.name}`);
          });
        } else {
