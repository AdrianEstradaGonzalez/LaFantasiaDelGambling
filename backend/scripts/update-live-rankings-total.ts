...
/**
 * Obtiene los partidos terminados de la jornada actual de La Liga
 */
async function getFinishedMatchesFromCurrentJornada(jornada: number): Promise<any[]> {
  try {
    console.log(`🔍 Buscando partidos terminados de la jornada ${jornada}...`);
    const { data } = await axios.get(`${API_BASE}/fixtures`, {
      ...
      params: {
        ...
        status: 'FT', // FT = Full Time (partido terminado)
      },
      timeout: 10000,
    });

    const fixtures = data?.response || [];
    console.log(`✅ Encontrados ${fixtures.length} partidos terminados`);
    return fixtures;
  } catch (error) {
    console.error('❌ Error obteniendo partidos terminados:', error);
    return [];
  }
}
...
async function updateLeagueRankings() {
  try {
    ...
    // 2. Obtener partidos terminados de la jornada actual
    const finishedFixtures = await getFinishedMatchesFromCurrentJornada(jornada);
    
    if (finishedFixtures.length === 0) {
      console.log('⏸️  No hay partidos terminados en la jornada actual. Esperando...\n');
      return;
    }

    console.log(`⚽ Partidos terminados: ${finishedFixtures.map((f: any) => `${f.teams.home.name} vs ${f.teams.away.name}`).join(', ')}\n`);

    // 3. Obtener todas las estadísticas de jugadores de los partidos terminados
    ...
    for (const league of primeraLeagues) {
      ...
      for (const member of league.members) {
        ...
        console.log(`  ✅ ${userName}: ${totalPoints} puntos (Total: ${totalPoints})`);
        updatedMembers++;
      }
    }
...
async function main() {
  console.log('\n' + '═'.repeat(70));
  console.log('🔄 WORKER DE ACTUALIZACIÓN DE RANKINGS (PARTIDOS TERMINADOS)');
  console.log('═'.repeat(70));
