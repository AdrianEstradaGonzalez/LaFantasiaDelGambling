...
/**
 * Obtiene los partidos EN CURSO de la jornada actual de Segunda División
 */
async function getLiveMatchesFromCurrentJornada(jornada: number): Promise<any[]> {
  try {
    console.log(`🔍 Buscando partidos en curso de la jornada ${jornada} (Segunda División)...`);
    const { data } = await axios.get(`${API_BASE}/fixtures`, {
      headers: HEADERS,
      params: {
        league: SEGUNDA_LEAGUE_ID,
        season: SEASON,
    ...
    console.log(`✅ Encontrados ${fixtures.length} partidos en curso (Segunda División)`);
    return fixtures;
  } catch (error) {
  ...
  try {
    ...
    for (const teamData of teamsData) {
      ...
      for (const playerData of players) {
        ...
        // Buscar el jugador en nuestra BD de Segunda División
        const dbPlayer = await (prisma as any).playerSegunda.findUnique({
          where: { id: playerId },
...
/**
 * Guarda o actualiza las estadísticas TEMPORALES de un jugador en la base de datos (Segunda División)
...
async function savePlayerStatsToDb(
  ...
  try {
    // Preparar los datos para guardar (formato compatible con PlayerSegundaStats)
    const statsData = {
      playerId,
      fixtureId,
      jornada,
    ...
    await (prisma as any).playerSegundaStats.upsert({
      where: {
...
/**
 * Obtiene la jornada actual consultando una liga de Segunda División
 */
async function getCurrentJornada(): Promise<number | null> {
  try {
    // Buscar cualquier liga de Segunda División para obtener su jornada actual
    ...
    if (league?.currentJornada) {
      console.log(`📅 Jornada actual obtenida de BD (Segunda): ${league.currentJornada}`);
...
/**
 * Actualiza los puntos EN VIVO de todas las ligas de Segunda División
 */
export async function updateLiveLeagueRankingsSegunda() {
  try {
    console.log('\n🚀 Iniciando actualización de rankings EN VIVO (SEGUNDA DIVISIÓN)...');
    ...
    console.log('\n💾 Guardando estadísticas EN VIVO en BD (PlayerSegundaStats)...');
    let savedStats = 0;
    ...
    console.log(`✅ ${savedStats} jugadores actualizados en PlayerSegundaStats (EN VIVO)\n`);

    // 4. Obtener todas las ligas de Segunda División con sus miembros y squads
    ...
    console.log(`\n🎉 Actualización EN VIVO completada (Segunda División). ${updatedMembers} miembros actualizados\n`);
    console.log('═'.repeat(70));
  } catch (error) {
    console.error('\n❌ Error en updateLiveLeagueRankingsSegunda:', error);
  }
}
...
async function main() {
  console.log('\n' + '═'.repeat(70));
  console.log('🔴 WORKER DE ACTUALIZACIÓN DE RANKINGS (SEGUNDA DIVISIÓN - PARTIDOS EN CURSO)');
  console.log('═'.repeat(70));

  await updateLiveLeagueRankingsSegunda();
